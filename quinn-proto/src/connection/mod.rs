...
impl Connection {
    ...
    pub fn ping(&mut self) {
        debug!("zzzzzzzz10 To Send PING to {:?} side: {:?}", self.remote_address(), self.side());
        ...
        if let Err(conn_err) = result {
            ...
            self.state = match conn_err {
                ...
                ConnectionError::TimedOut => {
                    debug!(
                        "Closing connection to {:?} at {:p} side {:?} reason: {:?}",
                        self.remote_address(),
                        self,
                        self.side(),
                        ConnectionError::TimedOut
    ...
    fn process_early_payload(
        ...
        for frame in frame::Iter::new(packet.payload.freeze()) {
            ...
            match frame {
                ...
                Frame::Close(reason) => {
                    debug!(
                        "zzzzzz Closing connection to {:?} at {:p} side {:?} reason: {:?}",
                        self.remote_address(),
                        self,
                        self.side(),
                        reason
    ...
    fn process_payload(
        ...
        if let Some(reason) = close {
            debug!(
                "zzzzzzz2 Closing connection to {:?} at {:p} side {:?} reason: {:?}",
                self.remote_address(),
                self,
                self.side(),
                reason
